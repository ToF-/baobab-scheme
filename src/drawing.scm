...
(define (draw-squares x y angle unit)
  (let* ((vert (+ angle (/ PI 2.0)))
    ...
    (begin
      ...
      (draw-square x-upper-left y-upper-left (- alpha (/ PI 2.0)) (distance x-upper-left y-upper-left x-top y-top))
      (draw-square x-top y-top (+ alpha (/ PI 2.0)) (distance x-top y-top x-upper-right y-upper-right))
      )))
...
(define (print-html-canvas)
  ...
  (draw-squares 500.0 600.0 (/ PI 48.0) 100.0)
  (format #t "  ctx.stroke();~%")
